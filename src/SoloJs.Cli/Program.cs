...
static int Run(string[] args)
{
    ...
    return args[0] switch
    {
        ...
        _ when args[0].EndsWith(".solojs", StringComparison.OrdinalIgnoreCase)
...
static int Watch(string[] args)
{
    ...
    outPath ??= Path.ChangeExtension(path, ".js");
...
static (string? Path, string? OutPath, bool Stdout) ParseCompileArgs(string[] args)
{
    ...
    if (path is null)
    {
        Console.Error.WriteLine("Usage: solojs compile <file.solojs> [--out file.js]");
...
{
    var result = SoloJsCompiler.Compile(File.ReadAllText(path), Path.GetFileNameWithoutExtension(path));
    if (!result.Ok)
    {
    ...
    outPath ??= Path.ChangeExtension(path, ".js");
    if (stdout) Console.Write(result.JavaScript);
    else
    {
        File.WriteAllText(outPath, result.JavaScript);
...
{
    Console.WriteLine("SoloJS 0.1.0 — SoloGem Solo5 (MIT)");
    return 0;
}
...
static void PrintHelp()
{
    Console.WriteLine(
        """
        SoloJS — easiest scripting by SoloGem (Solo5)

        Usage:
          solojs compile <file.solojs> [--out file.js] [--stdout]
          solojs watch <file.solojs>
          solojs studio
          solojs version
